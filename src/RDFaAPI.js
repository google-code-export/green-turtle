...
DocumentData.prototype.getProjection = function(subject, template) {
   ...
   return this._toProjection(snode,template);
}

DocumentData.prototype._toProjection = function(snode,template) {

   var projection = new Projection(this,snode.subject);
   for (var predicate in snode.predicates) {
      var pnode = snode.predicates[predicate];
      var values = [];
   ...
   if (property && value) {
      for (var subject in this.triplesGraph) {
         ...
         if (pnode) {
            for (var i=0; i<pnode.objects.length; i++) {
               if (pnode.objects[i].value==value) {
                  projections.push(this._toProjection(snode,template));
                  break;
               }
            }
         }
