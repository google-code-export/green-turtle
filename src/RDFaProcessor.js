...
RDFaProcessor.SCHEME = new RegExp("^[A-Za-z][A-Za-z0-9\+\-\.]*\:");
...
RDFaProcessor.prototype.parseURI = function(uri) {
   var match = RDFaProcessor.SCHEME.exec(uri);
   if (!match) {
   ...
   parsed.resolve = function(href) {
      ...
      } else if (RDFaProcessor.SCHEME.test(href)) {
         return href;
      } else {
         if (this.path.charAt(this.path.length-1)=='/') {
...
RDFaProcessor.prototype.parseGeneric = function(parsed) {
   if (parsed.schemeSpecificPart.charAt(0)!='/' || parsed.schemeSpecificPart.charAt(1)!='/') {
